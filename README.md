...
SKILL.md 不是一次性写成的：分节草稿（`drafts/`）→ 逐节校验（`sections.json` 规格）→ 失败节**定点回溯**（不整篇重写，好节原封不动）→ 全绿组装 → 终检写盘。跨次运行通过 `state.json` 记录尝试预算（3 次/节），超限告警"换方法而非重试"。

产线引擎独立维护于 [doc-pipeline](https://github.com/makefeier/doc-pipeline)（LangGraph 可选：未安装自动降级内置执行器）；设计与实测记录见 [value-lens/DESIGN.md](https://github.com/makefeier/value-lens/blob/main/DESIGN.md)。修改本仓库任一节草稿后：
